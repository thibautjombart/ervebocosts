...
p1 <- ggplot() +
  geom_bar_pattern(
    ...
    # Thinner stripes (ignored when pattern = NA)
    pattern_colour  = "black",
    pattern_fill    = "black",
    pattern_alpha   = 1,
    pattern_angle   = 45,

    # Make stripes thinner:
...
p2 <- ggplot() +
  geom_bar_pattern(
    data = x_plot,
    aes(
      x = reorder(fct_infreq(display_year), cost_class),
      y = height,
      fill = cost_class,
      pattern = pattern        # NA = no pattern, "stripe" = stripes
    ),
    stat = "identity",
    color = NA,
    ...
    # Make stripes thinner
    pattern_density = 0.02,  # lower = thinner bands
    pattern_spacing = 0.02,  # overall spacing
    pattern_size    = 0.05   # thinner stroke lines
  ) +
  scale_x_discrete(limits = rev) +
